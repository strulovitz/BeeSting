...
27. **`mv` not `cp` when renaming a file; `cp` when moving from Downloads into the repo.** Renaming a file within Downloads (e.g. from ElevenLabs's auto-generated `ElevenLabs_2026-04-19T09_28_50_Frank - Wise...mp3` to the clean `E01-block08-mechanism-y.mp3`) uses `mv` — one canonical name, no original left behind. Copying the cleanly-renamed file from Downloads INTO the repo (`audio/`, `clips/`, `elements/`) uses `cp` — Downloads keeps its working copy with the clean name for Nir's Premiere drag-in workflow per Lesson 12. End state: exactly one canonical name in Downloads, exactly one copy in the repo, zero originals-with-long-auto-generated-names lying around. Flagged by Nir 2026-04-19: *"each time you rename a file, you do not leave the original copy and make a new copy with a new name, you just rename the original file please."*

28. **Every clip is independent — alternating framings across consecutive clips + micro-beat (start → happen → land) in every clip.** Restating the three engagement tricks from `MECHANISM_SECTION_VISUAL_STYLE.md` because I drifted off them while proposing the recurring-villain story arc and had to be corrected. (a) Each clip stands alone — no dependency on what came before. (b) Consecutive clips must NOT share composition — wide → close → side → POV → top-down → isometric, alternate deliberately. (c) Every clip has a three-step micro-story inside its 6 seconds — something enters the frame, something changes or moves, something clicks into its final landing position. Even if the scene subject stays constant across the micro-beat, the motion arc is what keeps the eye alive — a static pose for 6 seconds is wasted credits.

29. **Before writing a Kling prompt, include a one-line quick-glance summary table for the whole shot plan.** Nir 2026-04-19: *"also yes please make a table with 1 line of description for each shot please."* The detailed shot-by-shot table with full micro-beat descriptions is necessary for generating the prompts, but it is hard to eyeball the whole mechanism at once. A compact 1-line-per-shot summary at the top of the shot plan (columns: Shot, One-line summary) lets Nir verify the full flow in under a minute before any credits are spent. Do this at the top of every Mechanism shot plan from now on — Y is now done, Z and W should also have this summary block.
