...
18. **Kling does NOT infer HUD overlays from the reference image — re-describe every persistent HUD element in the Kling prompt itself.** On 2026-04-18 the S13 Kling prompt said "the thermal HUD readouts tick slightly" but did NOT mention the crosshair. Kling dropped the crosshair in the generated clip even though it was in the reference image. **Rule:** whichever HUD elements must appear in the generated clip — crosshair, compass bar, corner readouts, timer, frame borders, color scale, warning banners — must be NAMED in the Kling prompt, every time, even if they are obviously in the reference image. Kling treats the reference image as a style/composition hint, not as a frame-one commitment. If the HUD element is not in the prompt, Kling may drop it or animate it weirdly.
