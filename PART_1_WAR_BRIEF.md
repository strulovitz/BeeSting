...
**Prompt format for every clip:**
```
[Visual]: describe scene and action
[Narration]: 'exact words spoken', [Vocal: deep, 45-year-old American male, calm, serious, slow pace]
[Sound effects]: specific sounds matching the action
[Music]: mood and style
```

**Example — Shot #1 (missile launch, 10s with headroom):**
```
[Visual]: A ballistic missile launches from a mobile military transporter at night. Massive exhaust plume billows across the launch pad. The missile accelerates upward, climbing into the dark starry sky, growing smaller. Camera stays low, looking up. Photorealistic, cinematic.
[Narration]: 'Israeli invention exists. Chinese production starting. This ballistic missile is not carrying a bomb. It is carrying something much worse.', [Vocal: deep, 45-year-old American male, calm, serious, slow pace]
[Sound effects]: deep rocket engine roar building from ignition, ground rumble and vibration, distant wind at altitude
[Music]: single ominous sustained low drone note, dark, tension building slowly, cinematic
```
...
| 6 | A5: wider corridor, doors | A6: night-vision green corridor | Corridor continues, view shifts to night-vision green tint | 7 (5+2) | "...every drone in history was either flown by a human on a radio — or too stupid to find a doorway." |
...
### 🎬 Production stack (revised 2026-04-16 — ALL audio now generated by Kling)

**Reference images:** OpenArt AI → ChatGPT Image 1.5 → 16:9 → Medium quality → 4 variants. Saved to `elements/` folder.

**Video clips WITH full audio (video + narration + sounds + music in ONE generation):**
Kling 3.0 Omni on OpenArt. Native Audio toggle ON. Two modes:
...
Settings: 16:9, 1080p, duration 3-15 seconds (any integer).

**Prompt structure for every clip ("Prompt Spine" method):**
```
[Visual]: describe the scene and action in detail
[Narration]: 'the exact words the narrator speaks', [Vocal: deep, 45-year-old American male, calm, serious, slow pace]
[Sound effects]: specific sounds matching the visual action
[Music]: mood and style of background music
```

**Voice consistency:** The SAME vocal profile tag goes in EVERY prompt: `[Vocal: deep, 45-year-old American male, calm, serious, slow pace]`. Simple adjectives only — Kling does not understand jargon like "History Channel register."

**Sound effects:** Be specific and match action. "Deep rocket engine roar, ground rumble" not "loud sounds." "Metallic echo in sealed bunker corridor" not "reverb." "Quiet drone propeller hum" not "drone sounds."

**Music:** Describe mood per clip. "Ominous sustained low drone, dark, building tension, cinematic" for the hook. "Silence" for the pause. "Grave, minimal, single low note" for the aftermath. Music instructions change per clip to match the emotional arc.

**What we do NOT need anymore:**
- ~~ElevenLabs~~ — narrator voice is generated by Kling natively
- ~~Separate music sourcing~~ — music is generated by Kling natively
- ~~Audio sync in Premiere~~ — audio comes out already synced to the video
- ~~Lip sync tools~~ — narrator is off-screen, no lips to sync

**Voice reference backup:** ElevenLabs sample saved at `voice/narrator_frank_reference.mp3` ("Frank - Wise, Deep and Motivational"). If Kling's generated voice quality isn't good enough after testing, fall back to: Kling for video + sounds + music, ElevenLabs for narrator voice only, sync in Premiere.

**Headroom rule:** Every clip generates 2-3 seconds LONGER than the narration needs. Trim in Premiere.

**Edit / assembly:** Adobe Premiere Pro — only used for: (1) stitching clips back to back, (2) trimming headroom, (3) three text-on-black frames (emoji equation, closing equations, end card). No effects, no transitions, no audio work.

**Backup models on OpenArt (if Kling fails on a specific clip):**
- Veo 3.1 — better cinematic quality, but limited to 4/6/8 second clips only, no voice binding
- Both available on OpenArt, use Kling as primary, Veo as backup
