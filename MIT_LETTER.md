...
8. **American drones die.** Here is what specifically happens to AeroVironment, General Atomics, Anduril, Skydio, Shield AI, and Kratos:
   - Ukraine has already proved cheap distributed drones destroy expensive centralized assets — Operation Spiderweb (40+ Russian strategic bombers destroyed by 117 FPV drones) and the Black Sea fleet ($250K drones sinking $50M warships) — both done *without any AI at all*.
   - The hive solves the bandwidth problem no other AI can: military drone radio collapses to 600 bps in contested environments, but the hive only passes 100-byte text between drones (each carrying its own tiny AI on a $95 Raspberry Pi), and it is also the only approach that works underwater on acoustic links.
   - Stuart Russell's *Slaughterbots* is no longer fiction — multiple Workers approach the target from different angles, the DwarfQueen combines their observations into a confident face-recognition match, and there is no sniper to catch or car bomb to trace.
   - A ten-drone autonomous face-recognizing assassination swarm — commercial FPVs plus the hive on $95 Raspberry Pis plus open-source AI — costs under $20,000 total, with no external server, no internet, and no signal to intercept.
