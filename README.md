...
> 🚫 **ABSOLUTE RULE — NO BORING TEXT-ON-COLOR TITLE CARDS. EVERY SHOT NEEDS REAL GRAPHICS.** Every shot is 6 seconds of moving video, never a still image. Punchy section-title moments ARE welcome — but they must be built into a rich animated visual with things moving (doors swinging shut, chisels carving, dust flying, stakes driving in with smoke rising), not served up as text-on-a-flat-color slide. A shot that reads *"Title card: Point 3. Train once, copy forever. Icon on silver-violet background"* is silent-film boredom and is forbidden. A shot where the same words appear carved into a vault door as the door swings shut, or burned into a newspaper headline as it slams onto a desk, is allowed and often great. If you catch a flat text-on-color slide in a shot plan, upgrade it into a real moving visual scene that happens to contain the words. See [`NEVER_TITLE_CARDS.md`](NEVER_TITLE_CARDS.md) for the full rule.
